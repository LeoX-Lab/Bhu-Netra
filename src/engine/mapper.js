...
const LOCS={
 ...
 owner:'owner|holder|vendee|khatedar|खातेदार|रायत|स्वामी|मालिक|मालक|ਮਾਲਕ|માલિક|মালিক|জমির মালিক|உரிமையாளர்|భూస్వామి|పట్టాదారు|ಭೂ ಒಡೆಯ|ഉടമസ്ഥൻ|ଧାରକ|जग्गा धनी|مالکان|مالک|second party',
 father:'father|पिता|ਪਿਤਾ|પિતા|পিতা|தந்தை|తండ్రి|ತಂದೆ|പിതാവ്|ପିତା|والد',
 seller:'vendor|seller|विक्रेता|ਵਿਕਰੇਤਾ|বিক্রেতা|விற்பவர்|విక్రేత|ಮಾರಾಟಗಾರ|വിറ്റവർ|ବିକ୍ରେତା|بائع|first party',
...
function mapFieldsFromText(txt, fields, geminiFields = null){
  ...
  if (geminiFields) {
    for (const [k, v] of Object.entries(geminiFields)) {
      if (v && fields[k] !== undefined) {
        fields[k] = { v: String(v), c: 0.99, edited: false, verified: false };
      }
    }
  ...
  const scanLabel=(labels,opts)=>{
    ...
    for(let li=0;li<LINES.length;li++){
      ...
      if(opts.nameLike){ if(!isName.test(v)||/विवरण|details|CHEER/i.test(v))continue;
        if(/^(?:of|and|the|by|for|de)\b/i.test(v))continue;   // captured a fragment, not a name - keep scanning
        v=v.replace(/^[^\w\u0900-\u0D7F]+/,'').replace(/^[a-z]\s+(?=[A-Z\u0900-\u0D7F])/,'').replace(/[\s*=~"«»|]+$/,'').replace(/\s+[a-z]{1,4}$/,''); }
      if(v)return v;
    }
    return '';
  };
  ...
  put('seller',scanLabel(LOCS.seller,{nameLike:true}).split(new RegExp(NAME_STOP,'i'))[0].replace(/\s+(?:GPA|PA|ATTY)$/i,''),.8);
  ...
  const cm=txt.match(/(?:rs\.?|₹|inr)\s*([\d.,]{3,})/i);if(cm)put('consideration',cm[1].replace(/^[.,]+/,'').replace(/[.,]$/,''),.84);
  /* e-stamp forms print "Consideration Price (Rs.) <amount>" - the rs/digits
   * rule above misses it when "(Rs.)" sits between label and amount. */
  if(!(fields.consideration&&fields.consideration.v)){
    const cp=txt.match(/consideration[^\n\d]{0,25}([\d.,]{4,})/i);
    if(cp)put('consideration',cp[1].replace(/^[.,]+/,'').replace(/[.,]$/,''),.8);
  }
  /* Hindi-stamp value form: "900/-रूपये" (amount suffixed with rupaye) */
  if(!(fields.consideration&&fields.consideration.v)){
    const rp=txt.match(/([\d.,]{3,})\s*\/?\s*[-–]?\s*(?:रुपये|रूपये|rupees)/i);
    if(rp)put('consideration',rp[1].replace(/^[.,]+/,'').replace(/[.,]$/,''),.78);
  }
  ...
  /* e-stamp certificates identify the document by a "Unique Doc. Reference"
   * (UP/Haryana SHCIL-style) rather than a deed no - OCR often garbles the
   * word "Reference" itself ("Relersnce"), so match the head tolerantly.
   * The reference is one long token that OCR splits with spaces - join the
   * uppercase/digit continuation chunks on the same line back onto it. */
  if(!(fields.deedNo&&fields.deedNo.v)){
    const URX=/unique\s*doc[a-z.]*\s*(?:ref|rel)[a-z.]*[^A-Za-z0-9]{0,8}([A-Za-z0-9][A-Za-z0-9\-\/!.\[\]]{5,39})((?:\s+[A-Z0-9\-]{4,})*)/i;
    const line=txt.split('\n').find(l=>URX.test(l));
    if(line){
      const lm=line.match(URX);
      const val=(lm[1]+(lm[2]||'')).replace(/\s+/g,'').replace(/[!.]+$/,'');
      if(val)put('deedNo',val,.75);
    } else {
      const ur=txt.match(/unique\s*doc[a-z.]*\s*(?:ref|rel)[a-z.]*[^A-Za-z0-9]{0,8}([A-Za-z0-9][A-Za-z0-9\-\/!.\[\]]{5,39})/i);
      if(ur)put('deedNo',ur[1].replace(/[!.]+$/,''),.75);
    }
  }
