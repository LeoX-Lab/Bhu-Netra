...
async function main(){
  ...
  try{
    ...
    // 2. scan (the with-1s khatauni that broke the old engine)
    const fd=new FormData();
    fd.append('file', new Blob([fs.readFileSync('/tmp/work/khatauni-with-ones.png')],{type:'image/png'}), 'khatauni.png');
    const scan=await (await fetch(BASE+'/api/scan',{method:'POST',body:fd})).json();
